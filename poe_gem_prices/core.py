...
def create_gem_object(gem_name: str, variant: str, price: float, quality: str, listed: int) -> None:
    """
...
def go_over_elements(data: dict) -> list:
    """
    Core function that goes over the collected data from the api and calls a function to create object for each gem
...
def save_data(list_of_objects: list) -> None:
    ...
    df = pd.DataFrame(sorted_save_lst)
